...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        cout << "Burst Time: ";
        cin >> burst[i];
        cout << "Priority (lower = higher): "; cin
        >> priority[i];
    }
    int completed = 0;
    int time = 0;
    // jumping to first arrival
    ...
    while (completed < n) {
        ...
        int idx = -1;
        int bestP = 99999;
        // it will pick highest priority available for
        (int i = 0; i < n; i++) {
            if (done[i] == 0 && arrival[i] <= time) { if
            (priority[i] < bestP) {
                bestP = priority[i];
                idx = i;
            }
    ...
    for (int i = 0; i < n; i++) {
        ...
        priority[i] << "\t" << waiting[i] << "\t" << tat[i] << endl;
        wsum += waiting[i];
        tsum += tat[i];
    }
